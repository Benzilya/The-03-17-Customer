...
func _input(event: InputEvent) -> void:
	if transitioning or not event.is_action_pressed("interact") or not hint.visible:
		return
...
func _next_night_unlocked() -> bool:
	var data := _read_save()
	return int(data.get("night", current_night)) >= current_night + 1
...
func _build_room() -> void:
	# Small employee rest room in the rear-right corner. The entrance faces the store.
	...
	lamp.position = Vector3(6.7,2.65,4.25)
	lamp.light_color = Color(0.72,0.70,0.58)
...
func _message(text:String,seconds:float) -> void:
	if game.has_method("show_message"):
		game.call("show_message", text, seconds)
