...
func _ready() -> void:
	...
	build_background(); build_menu(); build_settings_panel(); build_credits_panel(); update_continue_state(); apply_saved_settings()
...
func _process(delta: float) -> void:
	...
	if glitch_timer <= 0.0:
		...
		var tween: Tween = create_tween(); tween.tween_property(vignette, "color:a", randf_range(0.35, 0.62), 0.05); tween.tween_property(vignette, "color:a", 0.42, 0.12)
...
func build_background() -> void:
	...
	for i:int in range(8): var stripe:=ColorRect.new(); stripe.position=Vector2(705+i*53,245); stripe.size=Vector2(2,165); stripe.color=Color(.12,.16,.16,.45); add_child(stripe)
	...
	for y:int in range(0,720,4): var scanline:=ColorRect.new(); scanline.position=Vector2(0,y); scanline.size=Vector2(1280,1); scanline.color=Color(0,0,0,.10); scanline.mouse_filter=Control.MOUSE_FILTER_IGNORE; add_child(scanline)
...
func build_menu() -> void:
	...
	var title:=Label.new(); title.text=t("title"); title.add_theme_font_size_override("font_size",55); title.add_theme_color_override("font_color",Color(.88,.92,.90)); container.add_child(title)
	var subtitle:=Label.new(); subtitle.text=t("subtitle"); subtitle.add_theme_font_size_override("font_size",14); subtitle.modulate=Color(.58,.68,.64); container.add_child(subtitle)
	var spacer:=Control.new(); spacer.custom_minimum_size=Vector2(1,28); container.add_child(spacer)
	add_menu_button(container,t("new_shift"),start_new_game); continue_button=add_menu_button(container,t("continue"),continue_game); add_menu_button(container,t("settings"),show_settings); add_menu_button(container,t("credits"),show_credits); add_menu_button(container,t("quit"),quit_game)
	var footer:=Label.new(); footer.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); footer.position=Vector2(-430,-62); footer.size=Vector2(400,32); footer.horizontal_alignment=HORIZONTAL_ALIGNMENT_RIGHT; footer.text=t("security_footer"); footer.modulate=Color(.42,.49,.46); footer.add_theme_font_size_override("font_size",12); add_child(footer)
...
func build_settings_panel() -> void:
	...
	var note:=Label.new(); note.text=t("settings_note"); note.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; note.custom_minimum_size=Vector2(440,70); note.modulate=Color(.62,.68,.66); box.add_child(note)
...
func start_new_game() -> void:
	...
	if FileAccess.file_exists("user://save.json"): DirAccess.remove_absolute(ProjectSettings.globalize_path("user://save.json"))
	get_tree().change_scene_to_file(GAME_SCENE)
func continue_game() -> void:
	var night:int=_read_progress_night()
	if night>=6: get_tree().change_scene_to_file(NIGHT6_SCENE)
	elif night==5: get_tree().change_scene_to_file(NIGHT5_SCENE)
	elif night==4: get_tree().change_scene_to_file(NIGHT4_SCENE)
	elif night==3: get_tree().change_scene_to_file(NIGHT3_SCENE)
	elif night==2: get_tree().change_scene_to_file(NIGHT2_SCENE)
	else: get_tree().change_scene_to_file(GAME_SCENE)
func _read_progress_night()->int:
	if FileAccess.file_exists("user://save.json"):
		var file:=FileAccess.open("user://save.json",FileAccess.READ)
		if file!=null:
			var parsed:Variant=JSON.parse_string(file.get_as_text())
			if typeof(parsed)==TYPE_DICTIONARY:return int((parsed as Dictionary).get("night",1))
	if FileAccess.file_exists("user://save.cfg"):
		var cfg:=ConfigFile.new()
		if cfg.load("user://save.cfg")==OK:return int(cfg.get_value("progress","night",1))
	return 1
func update_continue_state()->void: continue_button.disabled=not FileAccess.file_exists("user://save.cfg") and not FileAccess.file_exists("user://save.json"); continue_button.tooltip_text=t("no_save") if continue_button.disabled else t("resume_save")
...
func on_volume_changed(value:float)->void:var bus:int=AudioServer.get_bus_index("Master");AudioServer.set_bus_volume_db(bus,linear_to_db(max(value/100.0,.001)));save_settings()
func on_fullscreen_toggled(enabled:bool)->void:DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_FULLSCREEN if enabled else DisplayServer.WINDOW_MODE_WINDOWED);save_settings()
func on_language_selected(index:int)->void:current_language="ru" if index==1 else "en";LOCALIZATION.set_language(current_language);get_tree().reload_current_scene()
func save_settings()->void:var config:=ConfigFile.new();config.load("user://settings.cfg");config.set_value("audio","master_volume",master_slider.value);config.set_value("display","fullscreen",fullscreen_check.button_pressed);config.set_value("display","language",current_language);config.save("user://settings.cfg")
func apply_saved_settings()->void:
	var config:=ConfigFile.new()
	if config.load("user://settings.cfg")==OK:master_slider.value=float(config.get_value("audio","master_volume",80.0));fullscreen_check.button_pressed=bool(config.get_value("display","fullscreen",false));on_volume_changed(master_slider.value);DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_FULLSCREEN if fullscreen_check.button_pressed else DisplayServer.WINDOW_MODE_WINDOWED)
